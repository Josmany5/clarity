// Comprehensive AI Test Suite - REAL API Testing
// Tests AI behavior with actual Gemini API calls
...
function buildSystemPrompt(context) {
  ...
  return `You are Wove, an AI productivity coach in Prose.
...
async function runTest(testCase, testNumber, totalTests) {
  ...
  try {
    ...
    if (testCase.category === 'json_leak_detection' && leaks.length > 0) {
      issues.push({
        type: 'json_leak',
    ...
    // Check data count accuracy
    if (testCase.category === 'data_count_accuracy') {
      const expectedCount = testCase.setup.tasks;
      // Parse count from AI response (looking for numbers)
      const countMatch = cleanedMessage.match(/(\d+)\s+task/i);
      const reportedCount = countMatch ? parseInt(countMatch[1]) : null;
...
async function main() {
  ...
  const resultsFile = path.join(RESULTS_DIR, `comprehensive-test-${timestamp}.json`);
